...
function customAssign(...objects) {
  ...
  let jointObjects = [];
  Object.assign(jointObjects, ...objects);
  for (let key in jointObjects) {
    if (typeof jointObjects[key] === "number") {
      jointObjects[key] = jointObjects[key].toString();
    }
  }
  return jointObjects;
}
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  return Object.freeze(resultObj) ? resultObj : "об'єкт не було заморожено";
}
...
function convertArrayToObj(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (arr.filter((element) => element[0] === key).length > 1) {
